...
std::auto_ptr<ConflictLogger> ConflictLogger::theConflictLoggerInstance;
Mutex ConflictLogger::m_mutex;
...
ConflictLogger& ConflictLogger::Instance()
{
    MutexLocker obtainLock(m_mutex);
    if (theConflictLoggerInstance.get() == 0)
    {
        theConflictLoggerInstance.reset(new ConflictLogger);
    }

    return *theConflictLoggerInstance;
}

bool ConflictLogger::AddEntry(const char* pszHash,
...
SyncLogEntry ConflictLogger::ReadFirstEntry(const char* pszHash)
{
	ParseFile(pszHash);
...
SyncLogEntry ConflictLogger::ReadEntry(cfg_t* pEntryCFG)
{
	// Assures that the section has been found, which means there is at least one section.
    assert(pEntryCFG != NULL);
	...
	SyncLogEntry sle(cfg_getstr(pEntryCFG, FILE_PATH_VARNAME),
		cfg_getstr(pEntryCFG, MOD_TIME_VARNAME), strModType.c_str()[0],0);
		///\todo don't know last parameter

    return sle;
}

bool ConflictLogger::ParseFile(const char* pszHash)
{
...
bool ConflictLogger::RemoveEntry(const char* pszHash, SyncLogEntry& sle)
{
